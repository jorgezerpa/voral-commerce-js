...
import { removeFromCart } from 'services/commerce'
...
export const CartCard = ({ item }) => {
  ...
  return (
    <div className='mx-auto flex gap-4 mb-5 justify-start align-middle'>
        <div className='relative  w-[100px] h-[100px] rounded-lg overflow-hidden bg-center bg-cover bg-no-repeat' style={{backgroundImage:`url("${item.image.url}")`}}>
        </div>

        <div className='w-[130px] sm:w-[200px]'>
            <h4 className='text-md sm:text-lg font-bold'>{item.name}</h4>
            <p className='text-sm font-semibold'>{item.price.formatted_with_symbol}</p>
